...
enum SHAPE { CIRCLE, RECTANGLE, INVALID, SHAPE_COUNT };

enum DIR { NONE = -1, UP = 0, DOWN = 1, LEFT = 2, RIGHT = 3, DIR_COUNT };

enum FROG_GOAL { SCHOOL, HOME };

enum CURRENT_GAME { FROGGER, POOP, GAME_COUNT };

enum GAME_STATUS { STANDBY, RUNNING, GAMEOVER };
...
void lcdDrawString(int x, int y, const char *string, short colorIndex,
                   int scale);
void setRandomSeed();
...
static unsigned short color[] = {RED,   YELLOW, GREEN, BLUE,
                                 WHITE, VIOLET, BLACK};
volatile int i = 0;
...
public:
    IObject() {}
    IObject(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape)
        : x(x),
          y(y),
          sizeX(sizeX),
          sizeY(sizeY),
    ...
    virtual bool isOutOfBound() {
        if (x < X_MIN || x + sizeX > X_MAX) return true;
        if (y < Y_MIN || y + sizeY > Y_MAX) return true;
        return false;
    }
    virtual bool isCollideWith(IObject *object) {
        if (object->getX() + object->getSizeX() < x ||
            object->getX() > x + sizeX)
            return false;
        if (object->getY() + object->getSizeY() < y ||
            object->getY() > y + sizeY)
            return false;
        return true;
    }
...
public:
    Player() {}
    Player(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape,
           int speed, DIR dir)
        : IObject(x, y, sizeX, sizeY, colorIndex, shape) {
        this->speed = speed;
        this->dir = dir;
    }
    virtual ~Player() {}
...
public:
    Frog() {}
    Frog(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape,
         int speed, DIR dir, FROG_GOAL goal)
        : Player(x, y, sizeX, sizeY, colorIndex, shape, speed, dir),
          goal(goal) {}
    ~Frog() {}
    ...
    void setDir() { IObject::setDir(static_cast<DIR>(Jog_key)); }
...
public:
    Obstacle() {}
    Obstacle(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape)
        : IObject(x, y, sizeX, sizeY, colorIndex, shape) {}
    ~Obstacle() {}
...
public:
    Car() {}
    Car(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape,
        int speed)
        : Obstacle(x, y, sizeX, sizeY, colorIndex, shape) {
        this->speed = speed;
        this->dir = RIGHT;
    }
    ~Car() {}
    ...
    void move() override {
        switch (dir) {
            case LEFT:
                x = max(x - speed, X_MIN);
                if (x == X_MIN) dir = RIGHT;
                break;
            case RIGHT:
                x = min(x + speed, X_MAX - sizeX);
                if (x == X_MAX - sizeX) dir = LEFT;
                break;
            default:
                break;
        }
...
public:
    ...
    void addCar() {
        if (currentCar >= MAX_CAR) return;
        cars[currentCar] =
            Car(randInt(X_MIN, X_MAX - CAR_SIZE_X), carY[currentCar],
                CAR_SIZE_X, CAR_SIZE_Y, currentCar + 1, RECTANGLE,
                randInt(MIN_CAR_STEP, MAX_CAR_STEP));
    ...
    void init() override {
        life = INITIAL_LIFE;
        score = INITIAL_SCORE;
        frog = Frog(PLAYER_INIT_X, PLAYER_INIT_Y, PLAYER_SIZE_X, PLAYER_SIZE_Y,
                    PLAYER_COLOR, RECTANGLE, PLAYER_STEP, RIGHT, SCHOOL);
        frog.draw();
        currentCar = 0;
        addCar();
...
public:
    Man() {}
    Man(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape,
        int speed, DIR dir)
        : Player(x, y, sizeX, sizeY, colorIndex, shape, speed, dir) {}
    ~Man() {}
...
public:
    Poop() {}
    Poop(int x, int y, int sizeX, int sizeY, short colorIndex, SHAPE shape)
        : Obstacle(x, y, sizeX, sizeY, colorIndex, shape) {
    ...
    void poopSpeedIncrease() { speed = min(speed + 1, MAX_POOP_STEP); }
...
public:
    ...
    void addPoop() {
        if (currentPoop >= MAX_POOP) return;
        currentPoop++;
    ...
    void init() override {
        ...
        currentPoop = 0;
        for (i = 0; i < MAX_POOP; i++) {
            poops[i] = Poop(0, INVALID_POOP_Y, POOP_SIZE_X, POOP_SIZE_Y,
                            POOP_COLOR, RECTANGLE);
        }
        man = Man(PLAYER_INIT_X, PLAYER_INIT_Y, PLAYER_SIZE_X, PLAYER_SIZE_Y,
                  PLAYER_COLOR, RECTANGLE, PLAYER_STEP, RIGHT);
        man.draw();
        addPoop();
    }
    void updatePerUser() override {
...
private:
    ...
    void selectGame() {
        switch (Jog_key) {
            case (2):
                currentGame = static_cast<CURRENT_GAME>(
                    max(static_cast<int>(currentGame) - 1, 0));
                isGameChanged = true;
                break;
            case (3):
                currentGame = static_cast<CURRENT_GAME>(
                    min(static_cast<int>(currentGame) + 1,
                        static_cast<int>(GAME_COUNT) - 1));
                isGameChanged = true;
                break;
            default:
                break;
...
public:
    ...
    void gameManagerInit() {
        ...
        while (status == STANDBY) {
            if (!isTitleDrawn) drawStandbyTitle();
            if (isGameChanged) drawStandbyGameName();
            if (Jog_key_in) {
                Jog_key_in = 0;
                if (Jog_key == 4) {
                    break;
...
void ScoreSound(int index1, int index2) {
    const int song1[][2] = {{G1, N4}, {G1, N4}, {E1, N8}, {F1, N8}, {G1, N4},
                            {A1, N4}, {A1, N4}, {G1, N2}, {G1, N4}, {C2, N4},
                            {E2, N4}, {D2, N8}, {C2, N8}, {D2, N2}};
    // const char *note_name[] = {"C1", "C1#", "D1", "D1#", "E1", "F1", "F1#",
    // "G1", "G1#", "A1", "A1#", "B1", "C2", "C2#", "D2", "D2#", "E2", "F2",
    // "F2#", "G2", "G2#", "A2", "A2#", "B2"};
    ...
    for (int row = 0; row < 5; row++) {
        for (int col = 0; col < 3; col++) {
            if (bitmap[row] & (1 << (2 - col))) {
                Lcd_Draw_Box(x + col * scale, y + row * scale, scale, scale,
                             color[colorIndex]);
            }
        }
    }
...
inline void drawRectangle(int x, int y, int sizeX, int sizeY,
                          short colorIndex) {
